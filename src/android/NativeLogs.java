...
public class NativeLogs extends CordovaPlugin {
    ...
    private  String getCrashLogsFromLogCat(int _nbLines) {

        LinkedList<String> logs = new LinkedList<String>();

        try {
            Process process = Runtime.getRuntime().exec("logcat -b crash");
            BufferedReader bufferedReader = new BufferedReader(
                    new InputStreamReader(process.getInputStream()));
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext)
        ...
        if (action.equals("getLog")) {
            ...
            callbackContext.success(log);
            return true;
        } else if (action.equals("getCrashLog")) {

            int nbLines = args.getInt(0);
            boolean bCopyToClipBoard = args.getBoolean(1);

            String log = getCrashLogsFromLogCat(nbLines);
